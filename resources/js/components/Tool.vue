<template>
  <div>
    ...
    <vue-nestable :value="nestableItems" @change="change()" @input="nestableItems = $event">
      <template v-slot="slot">
        <vue-nestable-handle
          ...
          <div class="w-2/3">
            ...
            <span class="font-lighter text-80 ml-4 text-sm">{{ slot.item[slug] }}</span>
          </div>
    ...
    change() {
      ...
      if(!this.disable){
        Nova.request().post('/nova-vendor/nest-tool/save-items', {
          ...
          Nova.$toasted.show(ref.__('Items Ordered!'), { type: 'success' });
        }).catch(function (error) {
          Nova.$toasted.show(ref.__('Error Occurred!'), { type: 'error' });
        });
      } else {
        Nova.$toasted.show(ref.__('Sorting Disabled!'), { type: 'error' });
      }
    },
  },
  mounted() {
    ...
    Nova.request().post('/nova-vendor/nest-tool/items', {
      ...
      Nova.$toasted.show(ref.__('Items Fetched!'), { type: 'success' });
    }).catch(function (error) {
      Nova.$toasted.show(ref.__('Error Occurred!'), { type: 'error' });
    });
  },
